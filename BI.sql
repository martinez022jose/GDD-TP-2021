...
CREATE TABLE FJGD_sql.BI_DIM_PC(
	...
	bi_pc_ram_fk varchar(15) NOT NULL,
...
CREATE TABLE FJGD_sql.BI_DIM_Fabricante(
 ...
 CREATE TABLE FJGD_sql.BI_DIM_RAM(
	...
	bi_ram_idFabricante varchar(30) NOT NULL
 )
...
ALTER TABLE FJGD_sql.BI_DIM_PC
ADD
	CONSTRAINT FK_BI_PC_RAM FOREIGN KEY (bi_pc_ram_fk) REFERENCES FJGD_sql.BI_DIM_RAM(bi_ram_idRam)
...
SELECT  
	...
	pc_idCodigo
FROM FJGD_sql.PC
GROUP BY pc_idCodigo,pc_idRam;

OPEN db_bi_cursor_PC
FETCH NEXT FROM db_bi_cursor_PC INTO @pc_idRam, @pc_Codigo

WHILE @@FETCH_STATUS = 0  
BEGIN
	BEGIN
		BEGIN TRY
			INSERT INTO FJGD_sql.BI_DIM_PC( bi_pc_ram_fk, bi_pc_codigo)
			VALUES (@pc_idRam, @pc_Codigo)
...
--JOIN FJGD_sql.Producto ON prod_codProducto = icomp_idProducto
--JOIN FJGD_sql.PC ON pc_idCodigo = prod_codProducto
--JOIN FJGD_sql.RAM ON ram_idRam = pc_idRam
--JOIN FJgd_sql.Disco ON disc_idDisco = pc_idDisco
WHERE icomp_idCategoria = 'PC' --and pc_idCodigo = '00yhid'
;
...
BEGIN  
	BEGIN TRY
		INSERT INTO FJGD_sql.BI_DIM_RAM(bi_ram_idRam, bi_ram_Tipo, bi_ram_Capacidad, bi_ram_Velocidad , bi_ram_idFabricante)
