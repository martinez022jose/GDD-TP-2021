...
SELECT  
	...
	CLIENTE_MAIL,
	CLIENTE_TELEFONO
FROM GD1C2021_V2.gd_esquema.Maestra
...
BEGIN
	BEGIN
				BEGIN TRY
							INSERT INTO GD1C2021_V2.dbo.Sucursal(sucu_Mail, sucu_Direccion, sucu_Telefono, sucu_Ciudad)
...
BEGIN  
	BEGIN TRY
					INSERT INTO GD1C2021_V2.dbo.Fabricante(fabr_codigo)
					VALUES (@fabr_codigo)
...
BEGIN  
	BEGIN TRY
				INSERT INTO GD1C2021_V2.dbo.Disco(disc_idDisco, disc_tipo, disc_capacidad, disc_velocidad, disc_idFabricante)
...
BEGIN  
	BEGIN TRY
			INSERT INTO GD1C2021_V2.dbo.RAM(ram_idRam, ram_Tipo, ram_Capacidad, ram_Velocidad ,ram_idFabricante)
...
BEGIN  
	BEGIN TRY
				INSERT INTO GD1C2021_V2.dbo.Micro(micr_idMicro, micr_Cache, micr_CantHilos, micr_Velocidad, micr_idFabricante)
...
BEGIN
	BEGIN TRY
				INSERT INTO GD1C2021_V2.dbo.Video(vide_Modelo, vide_Chipset, vide_Velocidad, vide_Capacidad, vide_idFabricante)
...
SELECT @cate_idPc = cate_idCategoria FROM GD1C2021_V2.dbo.Categoria WHERE cate_Descripcion = 'PC'
...
BEGIN  
	BEGIN TRY
			INSERT INTO GD1C2021_V2.dbo.PC(pc_idCodigo, pc_alto, pc_ancho, pc_profundida, pc_idRam, pc_idVideo, pc_idDisco, pc_idMicro)
			VALUES ( @pc_codigo, @pc_alto, @pc_ancho, @pc_profundidad, @pc_idRam, @pc_idVideo, @pc_idDisco, @pc_idMicro)
			-- TODO: Se agrega el 20%
			INSERT INTO GD1C2021_V2.dbo.Producto(prod_codProducto, prod_idCategoria, prod_Decripcion, prod_Precio) VALUES ( @pc_codigo,  @cate_idPc, 'PC ' + @pc_codigo, @prod_precio + (@prod_precio*0.20)); 
...
SELECT @cate_idAc = cate_idCategoria FROM GD1C2021_V2.dbo.Categoria WHERE cate_Descripcion = 'ACCESORIO'
...
BEGIN  
	BEGIN TRY
		INSERT INTO GD1C2021_V2.dbo.Accesorio(acce_idCodigo, acce_Descripcion) VALUES ( @acce_codigo, @acce_descripcion)
		-- TODO: Checkear que el precio seteado sea el correcto
		INSERT INTO GD1C2021_V2.dbo.Producto(prod_codProducto, prod_idCategoria, prod_Decripcion, prod_Precio) VALUES ( @acce_codigo,  @cate_idAc, @acce_descripcion, @prod_precio);
	END TRY
	BEGIN CATCH
		PRINT ERROR_MESSAGE()
	END CATCH
...
BEGIN  
	BEGIN TRY
				SELECT @comp_idSucursal = sucu_idSucursal FROM GD1C2021_V2.dbo.Sucursal WHERE sucu_Mail= @comp_SucursalMail
				INSERT INTO GD1C2021_V2.dbo.Compra(comp_NumeroCompra, comp_idSucursal, comp_FechaCompra, comp_GastoTotal) VALUES ( @comp_NumeroCompra, @comp_idSucursal, @comp_FechaCompra, @comp_GastoTotal)
...
BEGIN  
	BEGIN TRY
				...
				INSERT INTO GD1C2021_V2.dbo.ItemCompra(icomp_idCompra, icomp_idProducto, icomp_idCategoria, icomp_PrecioCompra, icomp_Cantidad) VALUES (@icomp_NumeroCompra, @icomp_idProducto, @icomp_idCategoria, @icomp_PrecioCompra, @icomp_Cantidad)
...
BEGIN  
	BEGIN TRY
		...
		IF NOT EXISTS (SELECT fact_Numero FROM Factura WHERE fact_Numero = @fact_NumeroFactura)
			BEGIN
			INSERT INTO GD1C2021_V2.dbo.Factura(fact_Numero, fact_idCliente, fact_fecha, fact_Total) VALUES (@fact_NumeroFactura, @fact_clieDNI, @fact_fecha, (@ifact_Cantidad * @ifact_PrecioProducto))
			END
		ELSE
			BEGIN
		...
		INSERT INTO GD1C2021_V2.dbo.ItemFactura([ifact_FacturaNumero], ifact_idProducto, ifact_idCategoria, ifact_Cantidad, ifact_PrecioProducto) VALUES (@fact_NumeroFactura, @ifact_idProducto, @ifact_idCategoria, @ifact_Cantidad, @ifact_PrecioProducto)
	END TRY
	BEGIN CATCH
	END CATCH
...
BEGIN  
	BEGIN TRY
				...
				SELECT @stoc_idSucursal = sucu_idSucursal FROM GD1C2021_V2.dbo.Sucursal WHERE sucu_Direccion = @stoc_SucursalDir 
				INSERT INTO GD1C2021_V2.dbo.Stock(stoc_idSucursal, stoc_idCategoria,stoc_idProducto,stoc_Cantidad) 
